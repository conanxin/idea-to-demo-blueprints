...
<!-- IDB-6H.1-pages-refresh-193135b: docs refresh marker -->


<!-- IDB-6H.2-first-build-section-reconcile -->

## IDB-6I Idea-to-Physical-Product Compiler Abstraction

IDB-6I 把 DIY Lamp Builder 从单一灯具 demo 抽象成一个通用实体产品编译器。

### 通用链路

Idea → Fixed Core → Variable Shell → Configurator → CAD Export → Print Validation → Physical Testing → Build Package → Sourcing → First Build Runbook

### 核心模式

- 固定功能内核：风险高、功能关键、涉及电气/热/结构边界的部分。
- 可变外壳：允许用户定制的造型、颜色、纹样、刻字和文化表达。
- 验证链路：CAD、打印、装配、照度/热/眩光、采购、首台样机报告。
